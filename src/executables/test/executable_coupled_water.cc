...
struct CoupledWaterProblem {
 public:
  ...
  void init() {
    ...
    pk_tree_list.sublist("coupled water").sublist("subsurface flow").set("PK type", "richards flow");
    pk_tree_list.sublist("coupled water").sublist("surface flow").set("PK type", "overland flow, pressure basis");
    ...
    // setup stage
    // common constants
    S->Require<double>("atmospheric_pressure",
                        Tags::DEFAULT, "coordinator");
    S->Require<AmanziGeometry::Point>("gravity",
            Tags::DEFAULT, "coordinator");
