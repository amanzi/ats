...
void ThermalConductivityEvaluator::Evaluate_(const State& S, const std::vector<CompositeVector*>& result)
{
  Tag tag = my_keys_.front().second;
  ...
  for (CompositeVector::name_iterator comp=result[0]->begin();
    ...
    const Epetra_MultiVector& cv =
      *S.GetPtr<CompositeVector>(Keys::getKey(domain_name,"cell_volume"),tag)->ViewComponent("cell",false);
    ...
    if (ice_cover_) {
      const AmanziGeometry::Point& zci = mesh->getCellCentroid(i_ice_max);
      z_ice = zci[2];

      const AmanziGeometry::Point& zcw = mesh->getCellCentroid(i_ice_min);
      z_w = zcw[2];
    }
...
//      exit(0);
  ...
  //  } // i
    ...
    for (int i=0; i!=ncomp; ++i)  {
      ...
      double hh = h[0][0]; //(h[0][0] == 0.) ? 1.5 : h[0][0]; // FoxDen
