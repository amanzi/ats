...
bool OverlandPressureFlow::UpdatePermeabilityData_(const Teuchos::Ptr<State>& S) {
  ...
  bool update_perm = S->GetFieldEvaluator("ponded_depth")->HasFieldChanged(S, name_);
  Teuchos::RCP<CompositeVector> pd = S->GetFieldData("ponded_depth","ponded_depth");
  ...
  update_perm |= S->GetFieldEvaluator("overland_conductivity")
