                                     ...
                                     Teuchos::RCP<TreeVector> g)
{
  ...
  if (!(std::abs(S_->get_time(tag_inter_) - t_old) < 1.e-4 * h)) {
    ...
    message << "SnowDistribution PK: ASSERT!: inter_time = " << S_->get_time(tag_inter_)
            << ", t_old = " << t_old << " --> diff = " << std::abs(S_->get_time(tag_inter_) - t_old)
            << " relative to " << h * 1.e-4
  ...
  if (!(std::abs(S_->get_time(tag_next_) - t_new) < 1.e-4 * h)) {
    Errors::Message message;
    message << "SnowDistribution PK: ASSERT!: inter_time = " << S_->get_time(tag_next_)
  ...
  db_->WriteVector(
    "k_s",
    S_next_->GetPtrW<CompositeVector>(Keys::getKey(domain_, "upwind_conductivity")).ptr(),
    true);
...
SnowDistribution::UpdatePreconditioner(double t, Teuchos::RCP<const TreeVector> up, double h)
{
  ...
  const CompositeVector& cell_volume =
    *S_next_->GetPtrW<CompositeVector>(Keys::getKey(domain_, "cell_volume"));
  CompositeVector cv_times_10(cell_volume);
