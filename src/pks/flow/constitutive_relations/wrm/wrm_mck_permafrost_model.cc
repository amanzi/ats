...
void
WRMMCKPermafrostModel::saturations(double pc_liq, double temperature, double (&sats)[3])
{
...
void
WRMMCKPermafrostModel::dsaturations_dpc_liq(double pc_liq, double temperature, double (&dsats)[3])
{
  if (pc_liq <= 0.) {
    dsats[0] = 0.;
    dsats[1] = 0.;
...
void
WRMMCKPermafrostModel::dsaturations_dpc_ice(double pc_liq, double temperature, double (&dsats)[3])
{
  double sr_ = wrm_->residualSaturation();
  AMANZI_ASSERT(sr_ >= 0.);
  AMANZI_ASSERT(sr_ < 1.);
  ...
  if (temperature >= T0_) {
    ...
    if (pc_liq <= 0.) {
      dsats[1] = 2 * (1. - sr_) * std::exp(-std::pow((temperature - T0_) / w_, 2)) *
                 (T0_ - temperature) / std::pow(w_, 2);
    } else {
      double s = wrm_->saturation(pc_liq);
      dsats[1] = 2 * (s - sr_) * std::exp(-std::pow((temperature - T0_) / w_, 2)) *
                 (T0_ - temperature) / std::pow(w_, 2);
    }
    dsats[2] = -dsats[1];
  }
