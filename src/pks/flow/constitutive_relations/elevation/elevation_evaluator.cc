...
ElevationEvaluator::Evaluate_(const State& S, const std::vector<CompositeVector*>& results)
{
  ...
  if (slope->hasComponent("boundary_face")) {
    const Epetra_Map& vandelay_map = slope->getMesh()->getMap(AmanziMesh::Entity_kind::BOUNDARY_FACE,false);
    const Epetra_Map& face_map = slope->getMesh()->getMap(AmanziMesh::Entity_kind::FACE,false);
