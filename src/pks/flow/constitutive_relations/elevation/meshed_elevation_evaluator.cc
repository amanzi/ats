                                                     ...
                                                     const std::vector<CompositeVector*>& results)
{
  ...
  if (parent_mesh->getManifoldDimension() == 3) {
    ...
    for (int c = 0; c != ncells; ++c) {
      ...
      elev_c[0][c] = parent_mesh->getFaceCentroid(domain_face, true)[2];
    ...
    if (elev->hasComponent("face")) {
      ...
      for (int f = 0; f != nfaces; ++f) {
        surface_nodes = surface_mesh->getFaceNodes(f);
    ...
    for (int c = 0; c != ncells; ++c) {
      ...
      elev_c[0][c] = parent_mesh->getCellCentroid(domain_cell, true)[2];

      // Figure out the upward normal using edges.
      // -- faces of the cell
      AmanziGeometry::Point normal(0., 0., 0.);
      AmanziMesh::Entity_ID_List faces;
      faces = parent_mesh->getCellFaces(domain_cell);
    ...
    if (elev->hasComponent("face")) {
      ...
      for (int f = 0; f != nfaces; ++f) {
        ...
        AmanziGeometry::Point x = parent_mesh->getFaceCentroid(domain_face, true);
        elev_f[0][f] = x[2];
      }
    }
  }
