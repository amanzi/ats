...
double FATES_PK::get_dt(){
  ...
  dt_ = std::min(t_photosynthesis_ + dt_photosynthesis_ - S_->get_time(tag_inter_), dt_photosynthesis_);
  dt_ = std::min(t_site_dym_ +  dt_site_dym_ - S_->get_time(tag_inter_), std::min(dt_, dt_site_dym_));
