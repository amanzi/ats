...
class PFT {
 public:
  ...
  virtual Teuchos::RCP<PFT> Clone() const { return Teuchos::rcp(new PFT(*this)); }
  virtual ~PFT() {}
  ...
  double
    storagecleaf2sw; //the ratio of storage carbon  in leaves compared to stem, given the same amount of drymass of leaf and stem (exluding storage)
  double
    storagecroot2sw; //the ratio of storage carbon in  root compared to stem, given the same amount of drymass of root and stem (exluding storage)
  double
    tar_leafstorageccon; // targe storage carbon concentration in leaves, g storage c / g leaf c (including storage)
