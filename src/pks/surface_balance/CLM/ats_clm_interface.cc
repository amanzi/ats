...
{
  ats_to_clm_ground_properties(latlon, sand[0], clay[0], color_index.data(), fractional_ground);
  return 0;
}
                ...
                double res_sat)
{
  ats_to_clm_et_controls(
    &beta_type, &veg_water_stress_type, &wilting_point, &field_capacity, &res_sat);
  return 0;
}
...
{
  clm_to_ats_mass_fluxes(evap_total[0],
                         evap_ground[0],
                         evap_soil[0],
                         evap_canopy[0],
                         tran_veg[0],
...
{
  clm_to_ats_diagnostics(swe[0], snow_depth[0], canopy_storage[0], Tskin[0], Tveg[0], Tsoil[0]);
