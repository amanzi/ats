...
// -- Setup data.
void
SurfaceBalanceELMKernels::Setup()
{
...
void
SurfaceBalanceELMKernels::SetupDependencies_(const Tag& tag)
{
  auto subsurf_mesh = S_->GetMesh(domain_ss_);
...
void
SurfaceBalanceELMKernels::Initialize()
{
  PK_Physical_Default::Initialize();
  InitializeELMKernels_(tag_next_);
...
SurfaceBalanceELMKernels::InitializeELMKernels_(const Tag& tag)
{
  ...
  //ATS::ELMKernels::set_zero_time(year);
  //ATS::ELMKernels::set_initial_state(temp, snow_depth);
  ...
  //ATS::ELMKernels::set_ground_properties(&latlon_arr[0][0], sand, clay, color_index, &pft_fraction[0][0]);

  // ELMKernels setup stage
  //ATS::ELMKernels::setup_begin();
  auto subsurf_mesh = S_->GetMesh(domain_ss_);
  ...
  //ATS::ELMKernels::set_dz(dz);
  //ATS::ELMKernels::set_et_controls(1, 2, 0.1, 1.0, 0.1);
  //ATS::ELMKernels::setup_end();
  //ATS::ELMKernels::set_dz(dz);
}


void
SurfaceBalanceELMKernels::InitializePrimaryVariables_(const Tag& tag)
{
...
bool
SurfaceBalanceELMKernels::AdvanceStep(double t_old, double t_new, bool reinit)
{
  ...
  //ATS::ELMKernels::set_wc(poro, sl);
  //ATS::ELMKernels::set_tksat_from_porosity(poro);
  //ATS::ELMKernels::set_pressure(pres, patm);
  ...
  //ATS::ELMKernels::set_met_data(met_sw, met_lw, met_prain, met_psnow, met_air_temp, met_rel_hum, met_wind_speed, patm);

  // set the start time, endtime
  //ATS::ELMKernels::advance_time(S_->get_cycle(tag), t_old, dt); // units in seconds
  ...
  Epetra_MultiVector& qE_cond = *S_->GetW<CompositeVector>(qE_cond_key_, tag, name_)
    .ViewComponent("cell", false);
  //ATS::ELMKernels::get_ground_energy_fluxes(qE_lh, qE_sh, qE_lw_out, qE_cond);
  ...
  Epetra_MultiVector& can_temp = *S_->GetW<CompositeVector>(can_temp_key_, tag, name_)
    .ViewComponent("cell", false);
  //ATS::ELMKernels::get_diagnostics(snow_swe, snow_depth, can_wc, surf_temp, can_temp, soil_temp);
  ...
  Epetra_MultiVector& ss_water_src = *S_->GetW<CompositeVector>(ss_water_src_key_, tag, name_)
    .ViewComponent("cell", false);
  //ATS::ELMKernels::get_total_mass_fluxes(surf_water_src, ss_water_src);
