...
void
SEBThreeComponentEvaluator::EvaluateField_(const Teuchos::Ptr<State>& S,
                             const std::vector<Teuchos::Ptr<CompositeVector> >& results)
{
  ...
  const auto& snow_volumetric_depth = *S->Get<CompositeVector>(snow_depth_key_).ViewComponent("cell",false);
  ...
  for (const auto& lc : land_cover_) {
    ...
    for (auto c : lc_ids) {
      ...
      if (area_fracs[0][c] > 0.) {
        ...
        double ss_water_source_l = flux.M_subsurf * area_to_volume * mol_dens[0][c]; // convert from m/m^2/s to mol/m^3/s
...
void
SEBThreeComponentEvaluator::EvaluateFieldPartialDerivative_(const Teuchos::Ptr<State>& S,
        Key wrt_key, const std::vector<Teuchos::Ptr<CompositeVector> > & results) {
  AMANZI_ASSERT(false);
}

void
SEBThreeComponentEvaluator::EnsureCompatibility(const Teuchos::Ptr<State>& S)
{
  if (!compatible_) {
    if (db_ == Teuchos::null) {
      // update the vo?
    ...
    if (diagnostics_) {
      ...
      S->Require<CompositeVector,CompositeVectorSpace>(snow_temp_key_, Tags::NEXT,  snow_temp_key_).Update(domain_fac_owned_snow);
    ...
    //S->Require<CompositeVector,CompositeVectorSpace>(area_frac_key_, Tags::NEXT).Update(domain_fac_3);
    compatible_ = true;
  }
}
...
void
SEBThreeComponentEvaluator::UpdateFieldDerivative_(const Teuchos::Ptr<State>& S, Key wrt_key)
{
