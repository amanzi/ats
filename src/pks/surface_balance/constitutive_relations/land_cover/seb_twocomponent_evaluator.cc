...
SEBTwoComponentEvaluator::SEBTwoComponentEvaluator(Teuchos::ParameterList& plist)
  : EvaluatorSecondaryMonotypeCV(plist), plist_(plist), compatible_(false), model_1p1_(false)
{
...
void
SEBTwoComponentEvaluator::Evaluate_(const State& S, const std::vector<CompositeVector*>& results)
{
  Tag tag = my_keys_.front().second;
    ...
    for (auto c : lc_ids) {
      ...
      if (area_fracs[0][c] > 0) {
        ...
        Relations::FluxBalance flux =
          Relations::UpdateFluxesWithoutSnow(surf, met, params, eb, mb, model_1p1_);
        ...
        if (model_1p1_)
          ss_water_source_l = flux.M_subsurf * area_to_volume * surf.density_w /
                              0.0180153; // convert from m/s to mol/m^3/s
        else
          ss_water_source_l =
            flux.M_subsurf * area_to_volume * mol_dens[0][c]; // convert from m/s to mol/m^3/s
      ...
      if (area_fracs[1][c] > 0.) {
        ...
        energy_source[0][c] +=
          area_fracs[1][c] * flux.E_surf * 1.e-6; // convert to MW/m^2 from W/m^2
  ...
  if (diagnostics_ && vo_.os_OK(Teuchos::VERB_HIGH)) {
    *vo_.os() << "----------------------------------------------------------------" << std::endl
    ...
    vecs.push_back(S.GetPtr<CompositeVector>(ponded_depth_key_, tag).ptr());
...
void
SEBTwoComponentEvaluator::EvaluatePartialDerivative_(const State& S,
                                                     const Key& wrt_key,
                                                     const Tag& wrt_tag,
...
void
SEBTwoComponentEvaluator::EnsureCompatibility_ToDeps_(State& S)
{
  if (!compatible_) {
    if (db_ == Teuchos::null)
    ...
    for (auto& dep : dependencies_) {
      ...
      } else if (dep.first == sg_albedo_key_ || dep.first == sg_emissivity_key_ ||
...
void
SEBTwoComponentEvaluator::UpdateFieldDerivative_(const State& S,
                                                 const Key& wrt_key,
                                                 const Tag& wrt_tag)
{
