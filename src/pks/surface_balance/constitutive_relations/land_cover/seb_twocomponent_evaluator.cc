...
void
SEBTwoComponentEvaluator::Evaluate_(const State& S,
                             const std::vector<CompositeVector*>& results)
{
  ...
  const auto& snow_depth = *S.Get<CompositeVector>(snow_depth_key_, tag).ViewComponent("cell",false);
...
SEBTwoComponentEvaluator::EnsureCompatibility_ToDeps_(State& S)
{
  if (!compatible_) {
    ...
    for (auto& dep : dependencies_) {
      ...
      } else if (dep.first == sg_albedo_key_ ||
