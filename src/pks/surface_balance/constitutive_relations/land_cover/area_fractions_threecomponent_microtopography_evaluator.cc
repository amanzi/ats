...
void
AreaFractionsThreeComponentMicrotopographyEvaluator::Evaluate_(const State& S,
        const std::vector<CompositeVector*>& result)
{
  auto tag = my_keys_.front().second;
  Epetra_MultiVector& res = *result[0]->ViewComponent("cell",false);

  const Epetra_MultiVector& pd = *S.Get<CompositeVector>(ponded_depth_key_, tag).ViewComponent("cell",false);
  const Epetra_MultiVector& sd = *S.Get<CompositeVector>(snow_depth_key_, tag).ViewComponent("cell",false);
  const Epetra_MultiVector& vsd = *S.Get<CompositeVector>(vol_snow_depth_key_, tag).ViewComponent("cell",false);
