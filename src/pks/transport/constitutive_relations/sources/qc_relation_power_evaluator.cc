...
QCRelationPowerEvaluator::QCRelationPowerEvaluator(Teuchos::ParameterList& plist) : EvaluatorSecondaryMonotypeCV(plist)
{
...
//   dependencies_.insert(KeyTag{ molar_density_key_, tag });
  first_src_key_ = Keys::readKey(plist, domain_, "primary source", "primary_source");
  dependencies_.insert(KeyTag{ first_src_key_, tag });
  second_src_key_ = Keys::readKey(plist, domain_, "power source", "power_source");
  dependencies_.insert(KeyTag{ second_src_key_, tag });
}

// Required methods from SecondaryVariableMultiEvaluator
void
QCRelationPowerEvaluator::Evaluate_(const State& S, const std::vector<CompositeVector*>& result) 
{
...
//     *S.Get<CompositeVector>(molar_density_key_, tag).ViewComponent("cell", false);
  ...
  for (AmanziMesh::Entity_ID c = 0; c != ncells; ++c) {  
    ...
    surf_src[0][c] = std::pow(primary_source[0][c], power_source[0][c]);
