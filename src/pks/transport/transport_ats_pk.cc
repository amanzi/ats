...
Transport_ATS::SetupTransport_()
{
  ...
  if (plist_->isSublist("source terms")) {
    ...
    if (sources_list->isSublist("geochemical")) {
      ...
      requireAtNext(water_src_key_, flow_tag_next_, *S_)
        .SetMesh(mesh_)
        ->SetGhosted(true)
        ->AddComponent("cell", AmanziMesh::Entity_kind::CELL, 1);
...
{
  // -- water flux
  requireAtNext(flux_key_, flow_tag_next_, *S_)
    .SetMesh(mesh_)
    ->SetGhosted(true)
...
Transport_ATS::Initialize()
{
  ...
  const Epetra_MultiVector& flux = *S_->Get<CompositeVector>(flux_key_, flow_tag_next_).ViewComponent("face", true);
  CheckInfluxBC_(flux);
...
Transport_ATS::AdvanceStep(double t_old, double t_new, bool reinit)
{
  ...
  S_->GetEvaluator(flux_key_, flow_tag_next_).Update(*S_, name_);
        ...
        if ((values[k] < 0) && (tcc_c[imap][c] > 1e-16)) {
          if (srcs_[m]->getType() == DomainFunction_kind::COUPLING) {
            const Epetra_MultiVector& flux_interface_ =
              *S_->Get<CompositeVector>("surface-surface_subsurface_flux", flow_tag_next_).ViewComponent("cell", false);
...
Transport_ATS::IdentifyUpwindCells_()
{
  ...
  S_->Get<CompositeVector>(flux_key_, flow_tag_next_).ScatterMasterToGhosted("face");
  const Epetra_MultiVector& flux = *S_->Get<CompositeVector>(flux_key_, flow_tag_next_).ViewComponent("face", true);
