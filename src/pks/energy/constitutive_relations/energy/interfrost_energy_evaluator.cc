...
void InterfrostEnergyEvaluator::Evaluate_(const State& S,
        const std::vector<CompositeVector*>& result) {
  auto tag = my_keys_.front().second;
  const Epetra_MultiVector& s_l = *S.Get<CompositeVector>("saturation_liquid", tag).ViewComponent("cell",false);
  const Epetra_MultiVector& n_l = *S.Get<CompositeVector>("molar_density_liquid", tag).ViewComponent("cell",false);
  const Epetra_MultiVector& u_l = *S.Get<CompositeVector>("internal_energy_liquid", tag).ViewComponent("cell",false);
  const Epetra_MultiVector& pres = *S.Get<CompositeVector>("pressure", tag).ViewComponent("cell",false);

  const Epetra_MultiVector& s_i = *S.Get<CompositeVector>("saturation_ice", tag).ViewComponent("cell",false);
  const Epetra_MultiVector& n_i = *S.Get<CompositeVector>("molar_density_ice", tag).ViewComponent("cell",false);
  const Epetra_MultiVector& u_i = *S.Get<CompositeVector>("internal_energy_ice", tag).ViewComponent("cell",false);

  const Epetra_MultiVector& phi = *S.Get<CompositeVector>("porosity", tag).ViewComponent("cell",false);
  const Epetra_MultiVector& phib = *S.Get<CompositeVector>("base_porosity", tag).ViewComponent("cell",false);
  const Epetra_MultiVector& u_rock = *S.Get<CompositeVector>("internal_energy_rock", tag).ViewComponent("cell",false);
  const Epetra_MultiVector& rho_rock = *S.Get<CompositeVector>("density_rock", tag).ViewComponent("cell",false);
  const Epetra_MultiVector& cell_volume = *S.Get<CompositeVector>("cell_volume", tag).ViewComponent("cell",false);
...
void InterfrostEnergyEvaluator::EvaluatePartialDerivative_(const State& S,
