...
void
ThreePhaseEnergyEvaluator::Evaluate_(const State& S,
        const std::vector<CompositeVector*>& result)
{
  ...
  for (CompositeVector::name_iterator comp=result[0]->begin();
    ...
    for (int i=0; i!=ncomp; ++i) {
      result_v[0][i] = model_->Energy(phi_v[0][i], phi0_v[0][i], sl_v[0][i], nl_v[0][i], ul_v[0][i],
              si_v[0][i], ni_v[0][i], ui_v[0][i],
              sg_v[0][i], ng_v[0][i], ug_v[0][i],
              rho_r_v[0][i], ur_v[0][i], cv_v[0][i]);
    }
  }
}
