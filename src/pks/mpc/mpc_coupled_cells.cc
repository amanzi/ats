...
void MPCCoupledCells::Setup()
{
  ...
  S_->Require<CompositeVector,CompositeVectorSpace>(A_key_, tag_next_).SetMesh(mesh_)->AddComponent("cell", AmanziMesh::CELL, 1);
  S_->Require<CompositeVector,CompositeVectorSpace>(y2_key_, tag_next_).SetMesh(mesh_)->AddComponent("cell", AmanziMesh::CELL, 1);
  ...
  // if (S_->GetEvaluator(A_key_, tag_next_).IsDifferentiableWRT(*S_, y2_key_, tag_next_))
    S_->RequireDerivative<CompositeVector,CompositeVectorSpace>(A_key_, tag_next_, y2_key_, tag_next_);
  ...
  S_->Require<CompositeVector,CompositeVectorSpace>(B_key_, tag_next_).SetMesh(mesh_)->AddComponent("cell", AmanziMesh::CELL, 1);
  S_->Require<CompositeVector,CompositeVectorSpace>(y1_key_, tag_next_).SetMesh(mesh_)->AddComponent("cell", AmanziMesh::CELL, 1);
  ...
  // if (S_->GetEvaluator(B_key_, tag_next_).IsDifferentiableWRT(*S_, y1_key_, tag_next_))
    S_->RequireDerivative<CompositeVector,CompositeVectorSpace>(B_key_, tag_next_, y1_key_, tag_next_);
...
void MPCCoupledCells::UpdatePreconditioner(double t, Teuchos::RCP<const TreeVector> up,
  ...
  if (dA_dy2_ != Teuchos::null &&
    ...
    Teuchos::RCP<const CompositeVector> dA_dy2_v = S_->GetDerivativePtr<CompositeVector>(A_key_, tag_next_, y2_key_, tag_next_);
  ...
  if (dB_dy1_ != Teuchos::null &&
    ...
    Teuchos::RCP<const CompositeVector> dB_dy1_v = S_->GetDerivativePtr<CompositeVector>(B_key_, tag_next_, y1_key_, tag_next_);
