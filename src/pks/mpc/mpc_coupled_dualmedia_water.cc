...
MPCCoupledDualMediaWater::MPCCoupledDualMediaWater(Teuchos::ParameterList& FElist,
...
void  MPCCoupledDualMediaWater::Setup(const Teuchos::Ptr<State>& S) {
...
void MPCCoupledDualMediaWater::Initialize(const Teuchos::Ptr<State>& S) {
  ...
  cvs_matrix_faces->SetMesh(mesh_matrix)->SetGhosted(true)
    ->AddComponent("face", AmanziMesh::FACE, Teuchos::rcpFromRef(mmap0), Teuchos::rcpFromRef(gmap0), 1);
  
  cvs_macropore_faces->SetMesh(mesh_macropore)->SetGhosted(true)
    ->AddComponent("face", AmanziMesh::FACE, Teuchos::rcpFromRef(mmap1), Teuchos::rcpFromRef(gmap1), 1);
...
void
MPCCoupledDualMediaWater::FunctionalResidual(double t_old, double t_new, Teuchos::RCP<TreeVector> u_old,
                            Teuchos::RCP<TreeVector> u_new, Teuchos::RCP<TreeVector> g) {
  // propagate updated info into state
  Solution_to_State(*u_new, S_next_);

  macro_flow_pk_->FunctionalResidual(t_old, t_new, u_old->SubVector(1),
...
int MPCCoupledDualMediaWater::ApplyPreconditioner(Teuchos::RCP<const TreeVector> u,
