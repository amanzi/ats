...
class PK_ConservationEquation : public Base_t {

public:

  PK_ConservationEquation(const Teuchos::RCP<Teuchos::ParameterList>& pk_tree,
  ...
  void Setup() {
    ...
    S_->template Require<CompositeVector, CompositeVectorSpace>(key_, tag_inter_, key_)
        .SetMesh(mesh_)
    ...
    if (is_source_) {
      ...
      if (!source_is_extensive_) {
        // source times volume
        std::string source_times_volume_key = source_key_+"_times_volume";
        Teuchos::ParameterList& source_times_vol = S_->FEList().sublist(source_times_volume_key);
        if (!source_times_vol.isParameter("tag"))
          source_times_vol.set("tag", tag_inter_);
        ...
        if (!source_times_vol.isParameter("dependencies")) {
          std::vector<std::string> deps{source_key_, cv_key_};
          source_times_vol.set("dependencies", Teuchos::Array<std::string>(deps));
        }
        source_key_ = source_times_volume_key;
      }
      ...
      if (comp == "cell") {
        ...
        Kokkos::parallel_reduce("ConservationEquation::ErrorNorm(cell) reduction",
                ...
                KOKKOS_LAMBDA(const int& c, ENorm_t& enorm) {
                  double local_enorm = abs(dt * du_v(c,0)) / (atol_ * cv_v(c,0) + rtol_ * abs(conserved_v(c,0)));
        ...
        Kokkos::parallel_reduce("ConservationEquation::ErrorNorm(face) reduction",
                ...
                KOKKOS_LAMBDA(const int& f, ENorm_t& enorm) {
                  ...
                  double local_enorm = fluxtol_ * dt * abs(du_v(f,0)) / (atol_ * cv_min + rtol_ * abs(conserved_min));
