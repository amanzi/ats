...
OverlandSourceFromSubsurfaceFluxEvaluator::IdentifyFaceAndDirection_(const State& S)
{
  ...
  for (int c = 0; c != ncells; ++c) {
    ...
    cells = subsurface->getFaceCells(domain_face, AmanziMesh::Parallel_kind::OWNED);
    AMANZI_ASSERT(cells.size() == 1);

    // -- Get directions
    AmanziMesh::Entity_ID_List faces;
    std::vector<int> fdirs;
    subsurface->getCellFacesAndDirections(cells[0], &faces, &fdirs);
                                                     ...
                                                     const std::vector<CompositeVector*>& result)
{
  ...
  if (volume_basis_) {
    ...
    for (int c = 0; c != ncells; ++c) {
      AmanziMesh::Entity_ID_List cells;
      subsurface->getFaceCells(
        (*face_and_dirs_)[c].first, AmanziMesh::Parallel_kind::OWNED, &cells);
      AMANZI_ASSERT(cells.size() == 1);
