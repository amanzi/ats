...
class EvaluatorColumnIntegrator : public EvaluatorSecondaryMonotypeCV {
 ...
 private:
  static Utils::RegisteredFactory<Evaluator,
                                  EvaluatorColumnIntegrator<Parser,Integrator>> reg_;
};
...
template<class Parser, class Integrator>
void
EvaluatorColumnIntegrator<Parser,Integrator>::Evaluate_(const State& S,
        const std::vector<CompositeVector*>& result)
{
  // collect the dependencies and mesh, and instantiate the integrator functor
  std::vector<const Epetra_MultiVector*> deps;
  for (const auto& dep : dependencies_) {
    deps.emplace_back(S.Get<CompositeVector>(dep.first, dep.second)
