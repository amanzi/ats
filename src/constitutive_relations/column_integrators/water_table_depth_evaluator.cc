...
ParserWaterTableDepth::ParserWaterTableDepth(Teuchos::ParameterList& plist,
        const KeyTag& key_tag)
{
  ...
  Key sat_key = Keys::readKey(plist, domain_ss, "saturation of liquid",
          "saturation_liquid");
...
int
IntegratorWaterTableDepth::scan(AmanziMesh::Entity_ID col, AmanziMesh::Entity_ID c, AmanziGeometry::Point& p)
{
  if ((*sat_)[0][c] < 1.0) {
    p[0] += (*cv_)[0][c];
    return false;
