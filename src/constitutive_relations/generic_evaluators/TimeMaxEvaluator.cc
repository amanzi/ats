    ...
    evaluated_once_(false)
{
  ...
  if (operator_ != "max" && operator_ != "min") {
    Errors::Message msg;
    msg << "TimeMaxEvaluator: " << my_keys_.front().first << ": invalid operator \""
        << operator_ << "\"";
...
void
TimeMaxEvaluator::Evaluate_(const State& S,
                            const std::vector<CompositeVector*>& result)
{
  ...
  const Epetra_MultiVector& dep = *S.Get<CompositeVector>(dependencies_.front().first,
          dependencies_.front().second).ViewComponent("cell", false);
