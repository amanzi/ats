...
def subsetVisFiles(in_directory, domain, out_directory,
    ...
    cycles : tuple or None
        Result of _parse_int_slice_or_list() for --cycles, or None.
    indices : tuple or None
        Result of _parse_int_slice_or_list() for --indices, or None.
    ...
    """
    # Both input and output use the same filenames; only the directory differs.
    # e.g. domain='surface' -> h5_name='ats_vis_surface_data.h5'
    #      domain='domain'  -> h5_name='ats_vis_data.h5'
    ...
    # 2. Apply cycle/time/index filter
    ...
    # 3. Select variables from the h5 file
    ...
    # 7. Write per-step XMFs for selected cycles
    # Step XMFs are named: {h5_name}.{N}.xmf  (e.g. ats_vis_surface_data.h5.42.xmf)
    ...
    # 8. Write master VisIt.xmf
    _write_visit_xmf(out_directory, h5_name, vf.cycles)

    # 9. Write subset H5 data file
    ...
    # 11. Copy mesh XMFs
    for suffix in [f'{mesh_name}.0.xmf', f'{mesh_name[:-3]}.VisIt.xmf']:
...
def _write_visit_xmf(out_directory, h5_name, cycles):
    ...
    ----------
    h5_name : str
        Full HDF5 filename, e.g. 'ats_vis_surface_data.h5'.
        Step XMFs are '{h5_name}.{N}.xmf'; master XMF is '{stem}.VisIt.xmf'.
    """
...
def main():
    ...
    parser.add_argument('domain', metavar='DOMAIN',
                        help='ATS domain name (e.g. "surface", "domain"). '
                             'Used to locate ats_vis_{DOMAIN}_data.h5.')
    ...
    var_group.add_argument('--include-vars', dest='include_vars',
                           nargs='+', metavar='VAR',
                           help='Only include these variables in output.')
    var_group.add_argument('--exclude-vars', dest='exclude_vars',
                           nargs='+', metavar='VAR',
                           help='Exclude these variables from output.')
