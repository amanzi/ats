...
def download_daymet(outdir, lat, lon, locid, start, end, vars=None):
    ...
    filename = os.path.join(outdir, raw_daymet_filename(lat, lon, locid, start, end))
...
def validate_start_end(start, end):
    ...
    daymet_end = datetime.date(2021, 12, 31)
...
def raw_daymet_filename(lat, lon, locid, start, end):
    if type(start) != datetime.date or type(end) != datetime.date:
        start = string_to_start_date(start)
        end = string_to_end_date(end)
    start, end = validate_start_end(start, end)
    return '{0}_{1}_{2}_{3}.dat'.format(locid, file_id(lat, lon), start.year, end.year)


def daymet_filename(locid, start, end):
    return '{0}_{1}_{2}.h5'.format(locid, start.year, end.year)
...
def get_argument_parser():
    ...
    parser.add_argument('locid', type=str,
                        help='Location name or id where daymet is downloaded')
...
if __name__ == '__main__':
    ...
    if args.raw_file is not None:
        ...
        daymet = download_daymet(args.directory, args.lat, args.lon, args.locid,
                                 start, end)
    ...
    if not args.download_only:
        ...
        filename_out = daymet_filename(args.locid, args.start, args.end) 
